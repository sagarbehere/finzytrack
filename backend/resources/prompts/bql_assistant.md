...
Today is {{TODAY}}. Current year = {{YEAR}}, last year = {{LAST_YEAR}}.
...
- FROM clause filters at the TRANSACTION level (e.g. FROM year = {{YEAR}})
...
- Dates are compared directly: date >= {{YEAR}}-01-01 (no quotes needed for dates)
...
- Income for a year: SELECT account, COST(SUM(position)) FROM year = {{YEAR}} WHERE account ~ 'Income' GROUP BY 1
