...
def test_detect_is_read_only_and_surfaces_action_required(tmp_path: Path):
    ...
    # config/recipes-relative paths, anchored by an absolute baseDir.
    assert {"path": "config/recipes/dashboards/d.json", "note": "dashboard"} in t.details["items"]
    assert t.details["baseDir"] == str((recipes).resolve().parent.parent)
...
def test_apply_runs_migration_and_then_self_retires(tmp_path: Path):
    ...
    # Normalized outcome the modal renders after applying (config/recipes-relative).
    assert {"path": "config/recipes/dashboards/d.json", "note": "upgraded"} in result["outcome"]["succeeded"]
    assert result["outcome"]["failed"] == []
    assert result["baseDir"] == str(recipes.resolve().parent.parent)
...
def test_partial_migration_downgrades_gate_to_dismissible_notice(tmp_path: Path):
    ...
    assert any(f["path"] == "config/recipes/dashboards/broken.json" and f["reason"] for f in result["outcome"]["failed"])
