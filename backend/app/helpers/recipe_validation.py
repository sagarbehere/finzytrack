...
#
# They are FUNCTIONS, not module-level constants, and must stay that way.
# backend/resources/schemas/ is gitignored and generated by
# scripts/sync_ai_reference.py, which app.ai.reference.autosync_dev runs at dev
# startup so a fresh clone self-heals. Computing these at import time defeated
# that: the schema was read during the router import in main.py, which happens
# before the FastAPI lifespan where autosync_dev runs, so a fresh clone died at
# import and the self-heal was unreachable. Nothing in this module may read the
...
def _enum(def_name: str) -> frozenset[str]:
    """The `enum` list of a top-level $def (e.g. ChartType)."""
    return frozenset(_defs()[def_name]["enum"])


def _discriminator_consts(union_def: str, prop: str) -> frozenset[str]:
...
@lru_cache(maxsize=1)
def valid_viz_types() -> frozenset[str]:
    """Top-level visualization `type` discriminators (chart / kpi / table / pivot)."""
    return _discriminator_consts("JsonRecipeVisualization", "type")


@lru_cache(maxsize=1)
def supported_chart_types() -> frozenset[str]:
    """Chart `chartType` values (bar / line / pie / treemap / …)."""
    return _enum("ChartType")


@lru_cache(maxsize=1)
def valid_param_types() -> frozenset[str]:
    """Recipe parameter `type` values."""
    return frozenset(_defs()["RecipeParameter"]["properties"]["type"]["enum"])


@lru_cache(maxsize=1)
def valid_query_engines() -> frozenset[str]:
    """Query step `engine` values."""
    return frozenset(_defs()["QueryStep"]["properties"]["engine"]["enum"])


@lru_cache(maxsize=1)
def valid_step_kinds() -> frozenset[str]:
    """Step `kind` discriminators (query / compute / transform)."""
    return _discriminator_consts("Step", "kind")
