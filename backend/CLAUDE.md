...
- **Every read-modify-write mutator MUST be decorated with `@_serialized_write`** (`app/core/ledger_manager.py`). The decorator holds the per-user write lock across the *whole* `parse → mutate → write → export` sequence, not just the file write. Without it, two concurrent mutations can each parse the same pre-write snapshot and the second rewrite silently clobbers the first (a lost update — not corruption). If you add a new method that calls `_parse_ledger()` and then `_write_and_export()` (or `_write_entries()`), it must carry this decorator. The lock is reentrant per thread, so the inner `_write_entries` acquire and any mutator-calls-mutator nesting are safe. Serialisation is verified by `tests/isolation/test_concurrent_writes.py::TestLostUpdatePrevention` (a forced-interleave test that fails if the parse escapes the lock).
...
**Reference:** `app/core/ledger_manager.py` — `_serialized_write` (RMW lock decorator), `_write_entries()`, `_do_write_entries()`, `append_entries()`; `app/write_lock.py` — `WriteLockManager` (reentrant `threading.RLock` + depth-guarded `portalocker` file lock); `app/core/beancount_engine.py` — `format_entries()` (padding filter).
