...
Usage:
    ...
    python debug_fetch.py server --profile axis-bank-savings --since-date 2026-03-01 --verbose

    # Test one account YAML file against one .eml file
    python debug_fetch.py rule --rule config/email_rules/axis-bank-savings.yaml \
                               --email-file /path/to/sample.eml

    # Show only unmatched emails
    python debug_fetch.py server --profile axis-bank-savings --since-date 2026-03-08 --unmatched-only
"""
import argparse
import sys
from pathlib import Path

# Allow running from the email_service directory
...
def cmd_server(args):
    ...
    config = load_config()
    registry = AccountProfileRegistry(config.rules_path)
        ...
        async for event_str in stream_fetch(
            ...
            if data_line.startswith('data: '):
                ...
                if evt['phase'] == 'complete':
                    from app.schemas.result_schemas import FetchResult
...
def cmd_rule_email(args):
    ...
    # Extract body
    from app.core.imap_client import _extract_body, _decode_header_value
