  ...
  <div>
    <div class="mb-6 flex items-start justify-between">
      ...
      <div class="flex rounded-md shadow-sm" role="group">
        <button
          ...
          :class="languageToggleClasses('sqlite')"
        >
          SQLite
        </button>
        <button
          @click="queryLanguage = 'beanquery'"
          :class="languageToggleClasses('beanquery')"
        >
          BQL
        </button>
      </div>
    ...
    <div class="overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-gray-200 dark:bg-gray-800/50 dark:shadow-none dark:ring-white/10 p-4 mb-4">
      ...
      <p v-if="!config?.ai?.llm?.is_configured" class="text-xs text-amber-600 dark:text-amber-400 mb-2">
        AI not configured. Set a model under <router-link to="/settings#ai-settings" class="underline underline-offset-2">Settings → AI</router-link> to enable query generation.
      </p>
      <textarea
        v-model="nlQuery"
    ...
    <div class="rounded-lg bg-white shadow-sm ring-1 ring-gray-200 dark:bg-gray-800/50 dark:shadow-none dark:ring-white/10 p-4 mb-4">
      ...
      <p class="mt-0.5 mb-1.5 text-xs text-gray-500 dark:text-gray-400">
        <template v-if="queryLanguage === 'sqlite'">
          ...
          <a href="https://docs.finzytrack.com/reference/querying-data/" target="_blank" rel="noopener noreferrer" class="text-indigo-500 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 underline underline-offset-2">database schema</a>
          for available tables and columns.
        </template>
        <template v-else>
          Write a BQL query against your ledger. See the
          <a href="https://docs.finzytrack.com/reference/querying-data/" target="_blank" rel="noopener noreferrer" class="text-indigo-500 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 underline underline-offset-2">BQL reference</a>
      ...
      <div class="border-b border-gray-200 dark:border-white/10">
        <nav class="flex -mb-px">
          <button
            ...
            :class="resultTabClasses('table')"
          >
            Table
          </button>
          <button
            @click="activeTab = 'chart'"
            :class="resultTabClasses('chart')"
          >
            Chart
          </button>
        </nav>
          ...
          <Listbox as="div" v-model="chartXColumn" class="min-w-[140px]">
            ...
            <div class="relative">
              <ListboxButton class="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-1.5 pr-2 pl-3 text-left text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-indigo-600 sm:text-sm/6 dark:bg-white/5 dark:text-white dark:outline-white/10 dark:focus-visible:outline-indigo-500">
                <span :title="chartXColumn || undefined" class="col-start-1 row-start-1 truncate pr-6">{{ chartXColumn || '-- select --' }}</span>
              ...
              <transition leave-active-class="transition ease-in duration-100" leave-to-class="opacity-0">
                <ListboxOptions class="absolute z-10 mt-1 max-h-60 min-w-full w-max overflow-auto rounded-md bg-white py-1 text-base shadow-lg outline-1 outline-black/5 sm:text-sm dark:bg-gray-800 dark:shadow-none dark:-outline-offset-1 dark:outline-white/10">
                  <ListboxOption :value="''" as="template" v-slot="{ active, selected }">
                    <li :class="[active ? 'bg-indigo-600 text-white dark:bg-indigo-500' : 'text-gray-900 dark:text-white', 'relative cursor-default py-2 pr-9 pl-3 select-none']">
                      <span :class="[selected ? 'font-semibold' : 'font-normal', 'block whitespace-nowrap']">-- select --</span>
                  ...
                  <ListboxOption v-for="col in resultColumns" :key="col" :value="col" as="template" v-slot="{ active, selected }">
                    <li :title="col" :class="[active ? 'bg-indigo-600 text-white dark:bg-indigo-500' : 'text-gray-900 dark:text-white', 'relative cursor-default py-2 pr-9 pl-3 select-none']">
                      <span :class="[selected ? 'font-semibold' : 'font-normal', 'block whitespace-nowrap']">{{ col }}</span>
                      <span v-if="selected" :class="[active ? 'text-white' : 'text-indigo-600 dark:text-indigo-400', 'absolute inset-y-0 right-0 flex items-center pr-4']">
                        <CheckIcon class="size-5" aria-hidden="true" />
                      </span>
                    </li>
          ...
          <div class="min-w-[200px]">
            <template v-if="chartType === 'pie'">
              <Listbox as="div" v-model="chartYColumnSingle">
                ...
                <div class="relative">
                  <ListboxButton class="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-1.5 pr-2 pl-3 text-left text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-indigo-600 sm:text-sm/6 dark:bg-white/5 dark:text-white dark:outline-white/10 dark:focus-visible:outline-indigo-500">
                    <span :title="chartYColumnSingle || undefined" class="col-start-1 row-start-1 truncate pr-6">{{ chartYColumnSingle || '-- select --' }}</span>
                    <ChevronUpDownIcon class="col-start-1 row-start-1 size-5 self-center justify-self-end text-gray-500 sm:size-4 dark:text-gray-400" aria-hidden="true" />
                  </ListboxButton>
                  <transition leave-active-class="transition ease-in duration-100" leave-to-class="opacity-0">
                    <ListboxOptions class="absolute z-10 mt-1 max-h-60 min-w-full w-max overflow-auto rounded-md bg-white py-1 text-base shadow-lg outline-1 outline-black/5 sm:text-sm dark:bg-gray-800 dark:shadow-none dark:-outline-offset-1 dark:outline-white/10">
...
<script setup lang="ts">
  ...
  /** Classes for the language toggle buttons (SQLite / BQL) */
  const languageToggleClasses = (lang: QueryLanguage) => [
    'px-4 py-2 text-sm font-medium border focus:outline-none focus:z-10',
    lang === 'sqlite' ? 'rounded-l-md' : '-ml-px rounded-r-md',
    queryLanguage.value === lang
  ...
  /** Classes for the results tab buttons (Table / Chart) */
  const resultTabClasses = (tab: 'table' | 'chart') => [
