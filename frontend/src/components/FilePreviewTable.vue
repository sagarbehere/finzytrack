<template>
  <div :class="fill
    ...
    : 'rounded-lg border border-gray-200 dark:border-gray-600 overflow-hidden'
  ">

    <!-- Sheet tabs (XLS with multiple sheets) -->
    <div
      v-if="sheets.length > 1"
      class="flex-none flex border-b border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 overflow-x-auto"
    >
      <button
        v-for="(sheet, i) in sheets"
        :key="i"
        class="px-3 py-1.5 text-xs whitespace-nowrap border-r border-gray-200 dark:border-gray-600 last:border-r-0 transition-colors"
        :class="activeSheet === i
      ...
      <table class="border-separate border-spacing-0 min-w-full text-xs">
        <thead>
          <tr>
            ...
            <th class="sticky top-0 left-0 z-20 bg-gray-50 dark:bg-gray-700 border-b border-r border-gray-200 dark:border-gray-600 px-2 py-1 min-w-[2.5rem] text-center text-gray-400 dark:text-gray-500 font-normal select-none">
              #
            </th>
        ...
        <tbody>
          <tr v-for="(row, ri) in displayRows" :key="ri">
            ...
            <td class="sticky left-0 z-10 bg-white dark:bg-gray-800 border-b border-r border-gray-200 dark:border-gray-600 px-2 py-px text-right text-gray-300 dark:text-gray-600 select-none font-mono min-w-[2.5rem]">
              {{ ri + 1 }}
            </td>
            <!-- Cells -->
            <td
              v-for="ci in colIndices"
              :key="ci"
              class="border-b border-r border-gray-100 dark:border-gray-700/60 px-2 py-px"
            >
              <div class="font-mono text-gray-700 dark:text-gray-300 max-w-[14rem] truncate">{{ row[ci] ?? '' }}</div>
            </td>
            <!-- Reverse row number (right) -->
            <td class="sticky right-0 z-10 bg-white dark:bg-gray-800 border-b border-l border-gray-200 dark:border-gray-600 px-2 py-px text-right text-orange-300 dark:text-orange-700 select-none font-mono min-w-[2.5rem]">
              {{ totalRows - ri }}
            </td>
          </tr>
        </tbody>
    ...
    <div
      ...
      class="flex-none px-3 py-1.5 border-t border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 text-xs text-gray-400 dark:text-gray-500 italic"
